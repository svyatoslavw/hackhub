...
export class AuthController {
  ...
  async emailConfirmation(@Body() dto: ConfirmationDto, @Res({ passthrough: true }) res: Response) {
    const result = await this.authService.sendEmailCode(dto)
  ...
  @ApiOperation({
    ...
    description: "Send sms with otp code"
  })
  @ApiResponse({ status: 200, type: ConfirmationResponse })
  @UsePipes(new ValidationPipe())
  @HttpCode(200)
  @Post("confirmation-phone")
  async phoneConfirmation(@Body() dto: ConfirmationDto) {
    return this.authService.sendPhoneCode(dto)
  }

  @ApiOperation({
    summary: "Profile confirmation",
    description: "Profile confirmation with otp code"
  })
  @ApiResponse({ status: 200, type: ConfirmationResponse })
  @UsePipes(new ValidationPipe())
  @HttpCode(200)
  @Post("confirmation-profile")
  async profileConfirmation(@Body() dto: LoginDto) {
    return this.authService.profileConfirmation(dto)
  }

  @ApiOperation({ summary: "Login by email or name" })
  @ApiResponse({ status: 200, type: AuthResponse })
  @UsePipes(new ValidationPipe())
  @HttpCode(200)
  @Post("login")
